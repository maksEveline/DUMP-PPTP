...
async def mail_btn():
    ...
    for btn in btns:
        ...
        if tip == "link":
            ...
            kb.add(
                InlineKeyboardButton(name, callback_data=f"mail_pod_cat_open:{value}")
            )
        elif tip == "position_open":
            kb.add(InlineKeyboardButton(name, callback_data=f"mail_pos_open:{value}"))
        elif tip == "contest_open":
            kb.add(
                InlineKeyboardButton(name, callback_data=f"mail_contest_view:{value}")
            )
...
async def user_menu(texts, user_id):
    ...
    if s["keyboard"] == "Inline":
        ...
        kb.append(
            InlineKeyboardButton("⚙️ Меню Администратора", callback_data="admin_menu")
        )
        ...
        for button in pr_buttons:
            keyboard.add(
                InlineKeyboardButton(
                    button["name"], callback_data=f"pr_button_user:{button['id']}"
                )
            )
...
async def contest_inl(texts, con_id, user):
    ...
    if user["count_refills"] >= contest["refills_num"] and "refills_num" in conds:
        count += 1
    if (
        len(await db.get_user_purchases(user["id"])) >= contest["purchases_num"]
        and "purchases_num" in conds
    ):
        count += 1
    ...
    if count == count_conds:
        kb.add(
            InlineKeyboardButton(
                texts.contest_enter, callback_data=f"contest_enter:{con_id}"
            )
        )
    else:
        kb.add(
            InlineKeyboardButton(
                f"❗ Вы не выполнили все условия! Выполнено {count} из {count_conds}",
...
async def profile_inl(texts):
    ...
    kb.append(
        InlineKeyboardButton(texts.last_purchases_text, callback_data="last_purchases")
    )
    kb.append(
...
def refill_open_inl(texts, way, amount, link, id, second_amount):
    ...
    kb.append(
        InlineKeyboardButton(
            texts.refill_check_inl,
            callback_data=f"check_opl:{way}:{amount}:{id}:{second_amount}",
        )
    )
...
async def refill_inl(texts):
    ...
    if yoomoney == "True":
        kb.append(
            InlineKeyboardButton(texts.yoomoney_text, callback_data="refill:yoomoney")
        )
    if lava == "True":
    ...
    if crystal == "True":
        kb.append(
            InlineKeyboardButton(texts.crystalPay_text, callback_data="refill:crystal")
        )
    if crypto == "True":
        kb.append(
            InlineKeyboardButton(texts.cryptoBot_text, callback_data="crypto_bot")
        )
    if payok == "True":
...
async def open_products(texts):
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton(texts.search, callback_data="search_pos"))

    for category in await db.get_all_categories():
        name = category["name"]
        cat_id = category["id"]
        keyboard.add(
            InlineKeyboardButton(name, callback_data=f"open_category:{cat_id}")
        )
...
async def open_pod_cat_positions(texts, pod_cat_id):
    ...
    keyboard.add(
        InlineKeyboardButton(
            texts.back, callback_data=f"open_category:{pod_cat['cat_id']}"
        )
    )
...
async def open_positions(texts, cat_id):
    ...
    for pod_cat in await db.get_pod_categories(cat_id):
        ...
        pod_cat_len = await db.get_positions_len(pod_cat_id)
        name = pod_cat["name"] + f" | {pod_cat_len} {texts.pcs}"
        if pod_cat_len == 0:
            continue
        keyboard.add(
            InlineKeyboardButton(name, callback_data=f"open_pod_cat:{pod_cat_id}")
        )
...
async def pos_buy_inl(texts, pos_id):
    ...
    keyboard.add(
        InlineKeyboardButton(
            texts.back, callback_data=f"open_category:{pos['category_id']}"
        )
    )
...
async def choose_languages_kb():
    ...
    for lang in langs:
        keyboard.add(
            InlineKeyboardButton(
                lang["name"], callback_data=f"change_language:{lang['language']}"
            )
        )
...
def choose_buy_items(pos_id, amount):
    ...
    kb.append(
        InlineKeyboardButton(
            f"✅ Да, хочу", callback_data=f"buy_items:yes:{pos_id}:{amount}"
        )
    )
    kb.append(
        InlineKeyboardButton(
            f"❌ Нет, не хочу", callback_data=f"buy_items:no:{pos_id}:{amount}"
        )
    )
